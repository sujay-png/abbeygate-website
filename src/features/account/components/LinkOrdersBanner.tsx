...
export function LinkOrdersBanner() {
  ...
  return (
    <div className="bg-brand-tint border-l-4 border-brand-primary p-4 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-r-md">
      <div className="flex items-center gap-3 text-sm text-brand-primary-dark font-medium">
        <svg className="w-5 h-5 text-brand-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <form action={action}>
        <button 
          type="submit" 
          disabled={isPending}
          className="whitespace-nowrap px-5 py-2 bg-brand-primary text-white text-sm font-medium rounded hover:bg-brand-primary-dark hover:shadow-sm transition-all disabled:opacity-50"
        >
